...
func ListNews(ctx context.Context) ([]*model.News, error) {
	...
	if err := db.WithContext(ctx).Order("publish_time desc").
		Limit(1000).Find(&news).Error; err != nil {
...
func ListNewsByFeedID(ctx context.Context, feedID int) ([]*model.News, error) {
	news := make([]*model.News, 0, 1024)
	if err := db.WithContext(ctx).Where("feed_id = ?", feedID).Order("publish_time desc").
		Limit(1000).Find(&news).Error; err != nil {
		log.Sugar.Errorw("get news error", "error", err)
		return nil, fmt.Errorf("get news error: %v", err)
	}
...
func GetLastNewsTime(ctx context.Context, feedID int) (time.Time, error) {
	news := &model.News{}
	if err := db.WithContext(ctx).Model(&model.News{}).
...
func DeleteOldNews(ctx context.Context) error {
	if err := db.WithContext(ctx).Where("publish_time < ?", time.Now().Add(-time.Hour*24*7)).
...
func DeleteNews(ctx context.Context, feedID int) error {
	if err := db.WithContext(ctx).Where("feed_id = ?", feedID).Delete(&model.News{}).Error; err != nil {
		log.Sugar.Errorw("delete news error", "error", err)
		return fmt.Errorf("delete news error: %v", err)
	}
